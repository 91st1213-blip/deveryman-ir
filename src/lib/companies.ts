...
function parseRow(row: string[]): Company | null {
  ...
  return { id, name, code, stock: n(row[14]), mCap: n(row[15]), roe: n(row[16]), pbr: n(row[17]), divYield: n(row[18]), shares: n(row[19]), prevClose: n(row[21]), change: n(row[22]), updatedAt: row[23]?.trim()||null };
}
